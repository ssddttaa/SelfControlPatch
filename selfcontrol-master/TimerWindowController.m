...
- (void)updateTimerDisplay:(NSTimer*)timer {
    ...
    [NSURLConnection sendAsynchronousRequest:urlRequest queue:[[NSOperationQueue alloc] init] completionHandler:^(NSURLResponse *response, NSData *data, NSError *connectionError) {
        if(connectionError)
        {
            ...
            NSDictionary * errorDict = @{NSUnderlyingErrorKey: underlyingError};
            if([[connectionError domain] isEqualToString:NSURLErrorDomain])
            {
                NSLog(@"YEETT URL DOMAIN");
                int timeHolder = INFINITY;
                [self responseHandler:&timeHolder];
                
            }
            else{
                NSLog(@"connection error:%@", connectionError);
                NSTimeInterval elapsedTime = [blockEndingDate_ timeIntervalSinceNow];
                
                int integerTime = (int) elapsedTime;
                [self responseHandler:&integerTime];
            }
        }
        else{
            ...
            NSString * dateString = [[NSString alloc]initWithData:responseData encoding:NSUTF8StringEncoding];
            ...
            NSTimeInterval elapsedTime = [blockEndingDate_ timeIntervalSinceDate:dateFromString];
...
-(void) responseHandler: (int*) elapsedTime
{
